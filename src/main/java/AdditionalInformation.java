...
class AdditionalInformation {
    ...
    ArrayList<String> getDataFromAdditionalFiles() {
        ...
        if (Files.exists(paLst)) {
            listAdditFiles.add(parseText(readFile.getListFile(ExtensionHelper.changeNameExtension(filePath, "lst"))));
        ...
        if (Files.exists(paInf)) {
            listAdditFiles.add(parseText(readFile.getListFile(ExtensionHelper.changeNameExtension(filePath, "inf"))));
        } else listAdditFiles.add("Инфофайл не найден.");
